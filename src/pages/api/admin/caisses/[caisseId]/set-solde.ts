...
/**
 * POST /api/admin/caisses/[caisseId]/set-solde
 *
 * Fixe directement le solde d'une caisse (quelque soit son type/kind) à une
 * valeur donnée. Crée une transaction CREDIT ou DEBIT d'ajustement pour la
 * traçabilité.
 *
...
async function handler(
  ...
  try {
    ...
    const caisseId = String(req.query.caisseId || '');
    if (!mongoose.isValidObjectId(caisseId)) {
      return res.status(400).json({ success: false, error: 'ID caisse invalide' });
    }
    ...
    console.error('[POST /api/admin/caisses/[caisseId]/set-solde]', error);
